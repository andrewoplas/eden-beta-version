        ...
        private static class DetectorListener implements FrameDetector.ImageListener, FrameDetector.FaceListener {
            ...
            public void onImageResults(List<Face> faces, Frame frame, float v) {
                if (faces.size() > 0 && Emotion.detect) {
                    ...
                    Emotion.addBrowFurrow(new Emotion().new Pair<String,Float>(time, face.expressions.getBrowFurrow()));
                    Emotion.addEngagement(new Emotion().new Pair<String,Float>(time, face.emotions.getEngagement()));
                    Emotion.addAttention(new Emotion().new Pair<String,Float>(time, face.expressions.getAttention()));
                    Emotion.addJoy(new Emotion().new Pair<String,Float>(time, face.emotions.getJoy()));
                    ...
                    String time = StopWatch.getTime();
                    Emotion.addBrowFurrow(new Emotion().new Pair<String,Float>(time,0f));
                    Emotion.addEngagement(new Emotion().new Pair<String,Float>(time,0f));
                    Emotion.addAttention(new Emotion().new Pair<String,Float>(time,0f));
                    Emotion.addJoy(new Emotion().new Pair<String,Float>(time,0f));
                }
